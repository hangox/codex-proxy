...
- 撤回 `variant_mismatch` 的族 B 分类（团队复核裁决，非遗漏）：此前把它和 `session_mismatch`/`model_mismatch` 一起归进"忽略 marker、丢弃 state、静默继续"的族 B，但复核发现这对 `variant_mismatch` 是错的——命中时 state 本身完好、内容仍可解密恢复，只是 variant 指纹算不一致，很可能是翻译层 `instructions`/`tools` 序列化没能保证 `variant-hash.ts` 自己写明的确定性前提，而不是"钥匙真配错了门"；归进族 B 等于把一份本可完整恢复的历史当垃圾扔掉，且用户全程看不到任何报错，比继续 409 更危险。且即便重分类也治不了实际症状：qa 红基线实证真实客户端在过期前先连续撞 7 次 `variant_mismatch`，大概率发生在普通对话轮次而非 `/compact` 请求上，族 A/B 的"仅在 compact 请求或无条件放行"都覆盖不到真正需要修的地方。正确修法大概率是修翻译层的确定性 bug 或引入新的 remedy 类别，不是重分类——在 qa 定位出真实触发原因前，`variant_mismatch` 继续默认 409。同步改写受影响的 e2e 用例（原断言 200+占位替换，现断言 409）与分类函数穷举测试（`src/routes/shared/opaque-compact-state.ts`、`tests/unit/routes/opaque-compact-state.test.ts`、`tests/e2e/messages.test.ts`、`tests/e2e/opaque-compact-lifecycle.test.ts`）
