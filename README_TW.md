...
- **WebSocket 介面** — `/v1/responses` 支援用戶端 WebSocket 串流直連（Bearer 鑑權），HTTP POST + SSE 作為回退保留
- **第三方 API Keys** — 支援多協定、依模型直通
...
- **備援上游** — 當所有帳號皆不可用時，自動啟用一個可編輯的兜底 API Key
...
### 🛡️ 反偵測與協議偽裝
- **Rust Native TLS** — 內建 reqwest + rustls native addon，TLS 指紋與真實 Codex 客戶端精確一致，跨 Windows / macOS / Linux（含 Alpine）
