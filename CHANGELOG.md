...
- Claude Code 新增实验性 stateful opaque compact bridge：`/compact` 仅调用一次上游 `/codex/responses/compact`，完整 opaque output（含 reasoning / encrypted content）只保存在进程内存，客户端仅接收短签名 marker；后续请求按 session / model / account / variant / digest 严格校验并恢复状态，重复 compact 会保留并去重完整尾部工具链，冲突、残缺、过期或账号不可用时 fail-closed。状态存储带 TTL、条目数和总字节上限且不落盘，日志仅记录结构计数与耗时。功能通过 `model.claude_code_opaque_compact_experimental` 显式启用，产品默认、旧配置缺字段与 Dashboard 默认均保持 `false`；传统 `claude_code_compact_bridge` 也继续默认关闭（`src/routes/messages.ts`、`src/routes/shared/codex-compact-service.ts`、`src/routes/shared/opaque-compact-bridge.ts`、`src/routes/shared/opaque-compact-state.ts`、`src/proxy/codex-api.ts`、`src/config-schema.ts`、`config/default.yaml`、`web/src/components/GeneralSettings.tsx`）。
