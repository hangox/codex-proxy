...
- **确定性的工具 schema 校验错误不再被当成可重试的 5xx 无限退避，导致交互式会话卡死。** 真实复现：Claude Code 内置 Artifact 工具的 `doc_id` 字段 pattern 用了 Unicode 属性转义正则，上游判定为非法 regex，报 `Invalid schema for function 'Artifact': ... is not a 'regex'.`。原始 HTTP 层报成 502；真实交互式会话默认走的 WebSocket/SSE 事件路径则以未归类的 `invalid_function_parameters` code 报出、同样兜底成 502——这类错误重放多少次都是同一个结果，不是传输抖动，但 502 落在 `withRetry` 的可重试区间，Claude Code 客户端自己的退避重试也是如此，于是交互式会话被无限重试拖死（QA 用真实账号复现到 attempt 10/10 才放弃）。新增共享分类 `classifyRawUpstreamError`（`src/proxy/error-classification.ts`），命中这类文本特征时把状态码改写成 400 并标记 `retryable: false`，不影响真正的传输层 5xx；已同时接入原始 HTTP 错误路径（`codex-api.ts`、`anthropic-upstream.ts`、`gemini-upstream.ts`、`openai-upstream.ts`、`responses-upstream.ts`）与 SSE/WS 事件错误路径（`codex-api-error-from-event.ts` 的 `codexApiErrorFromEvent`），两条此前各自维护、互相漏覆盖的分类逻辑统一到同一份判据。

## [2.0.104] - 2026-08-18

### Fixed

- **同步 upstream/master 最新提交（`d9e7846`）。** 主要带入上游对上游 WebSocket 首帧 `server_is_overloaded` 的 503 分类处理与 `promote-dev-to-master` 分页读取 CI 结果的修复；解决 4 处内容冲突（均为双方在同一位置各自新增独立错误码分支/独立测试用例，双方内容均保留）。合并过程中发现并修复一处隐私回归：新引入的 503 分支此前会把明文账号 entryId/email 直接写入日志，绕过了本仓库既有的 `safeLog` 脱敏开关，现已改为复用同函数其余分支使用的脱敏账号标识。
...
- **压缩明细里「降级压缩」行的耗时改为真正打上游的时间。** 此前列表用 `duration_ms`（降级决定后到终止的整段，含换号/退避），看起来像预判时间。现在 `fallback_render` 落盘 `upstream_ms`（仅普通生成真正调用上游的区间），列表优先显示它；没打到上游的 pre_stream 不伪造 0。预判降级行仍用自己的估算耗时。
...
- **★ 修一类正在生产反复发生的误降级：预算预判把本来能压缩成功的会话判成了超限。** 生产取证（20 小时窗口）：估算 868679 token 触发预算闸门（预算 390000）直接跳过上游，而该请求**真实只有 267633 token**——高估 3.25 倍，真实值远在预算内，这次压缩本来完全能成功。四次可分类的尝试**全部是误判**。根因是字节比例粗筛（cheap 估算）：base64 图片的字节数和真实 image token 数几乎无关（图片按像素网格计费），而含图片会跳过精确分词器、分词器超时熔断也会回退，两条路都落到这个估算器上。修法是**按估算来源区分信任度**而不是统一放宽：`cheap` 来源的预判阈值放宽到预算的 4 倍，`precise` / `precise_extrapolated` 逐字节保持原判断不变（误差 <1.5%，可信，没理由放宽）——cheap 只在"我们算不准"的场景触发，拿它做硬拦截决策本来就不合适；而真超限的兜底是安全的：这类失败是同步 400、不标记账号状态、不伤账号。**4 不是校准结果，是有 3.25x 实测下界支撑并留了余量的起点值**，取证样本按会话去重后只有 2 个独立会话，不足以定任何分组常数，因此同时新增 `has_image` / `image_bytes` / `text_bytes` 埋点，等攒够生产数据后重新审视这个倍数、并分别校准文本和图片两个换算比例。**没有动**逐模型实测校准出来的预算表（预算侧是可信的，问题在估算侧），也**没有动** 2000ms 熔断与 20% 外推门槛——同一个会话前后走进两条不同分类机制的异常尚未查明，不排除熔断路径的根源其实也是图片，在验证之前不碰。
