...
describe("codex-api headers", () => {
  ...
  describe("Compact path", () => {
    it("forwards identity, context headers, metadata, and strips internal fields", async () => {
      ...
      expect(headers["x-client-request-id"]).toMatch(/^cp_[0-9a-f]{32}$/);
      expect(headers.session_id).toBe(headers["x-client-request-id"]);
      ...
      expect(body.service_tier).toBe("priority");
      expect(body.prompt_cache_key).toBe(headers["x-client-request-id"]);
    ...
    it("derives the default compact window identity from prompt_cache_key", async () => {
      ...
      expect(headers["x-client-request-id"]).toMatch(/^cp_[0-9a-f]{32}$/);
      expect(headers["x-codex-window-id"]).toBe(`${headers["x-client-request-id"]}:0`);
      expect(body.client_metadata?.["x-codex-window-id"]).toBe(headers["x-codex-window-id"]);
      expect(transport.post).not.toHaveBeenCalled();
    });
  });
