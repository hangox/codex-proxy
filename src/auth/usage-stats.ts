...
export interface UsageSnapshot {
  ...
  totals: {
    ...
    /** Estimated USD cost from the local official API price catalog. */
...
export class UsageStatsStore {
  ...
  constructor(persistence?: UsageStatsPersistence) {
    ...
    if (this.baseline.estimated_cost_usd == null) this.baseline.estimated_cost_usd = 0;
    ...
    if (!loaded.baseline && this.snapshots.length > 0) {
      ...
      this._pendingRecovery = {
        ...
        estimated_cost_usd: last.estimated_cost_usd ?? 0,
      };
    }
  }
  ...
  recoverBaseline(pool: AccountPool): void {
    ...
    this.baseline = {
      ...
      estimated_cost_usd: Math.max(0, (this._pendingRecovery.estimated_cost_usd ?? 0) - live.estimated_cost_usd),
    };
  ...
  private poolTotals(pool: AccountPool): { input_tokens: number; output_tokens: number; cached_tokens: number; image_input_tokens: number; image_output_tokens: number; image_request_count: number; image_request_failed_count: number; estimated_cost_usd: number; request_count: number; active_accounts: number; total_accounts: number } {
    ...
    for (const entry of entries) {
      ...
      estimated_cost_usd += entry.usage.estimated_cost_usd ?? 0;
  ...
  recordSnapshot(pool: AccountPool): void {
    ...
    if (lastSnapshot) {
      ...
      const baselineCost = this.baseline.estimated_cost_usd ?? 0;
      ...
      const lastCost = lastSnapshot.totals.estimated_cost_usd ?? 0;
      const prevLive = {
        ...
        estimated_cost_usd: lastCost - baselineCost,
      };
      if (live.input_tokens < prevLive.input_tokens ||
          ...
          live.estimated_cost_usd < prevLive.estimated_cost_usd) {
        this.baseline = {
          ...
          estimated_cost_usd: baselineCost + Math.max(0, prevLive.estimated_cost_usd - live.estimated_cost_usd),
        };
      }
    }
    ...
    this.snapshots.push({
      ...
      totals: {
        ...
        estimated_cost_usd: (this.baseline.estimated_cost_usd ?? 0) + live.estimated_cost_usd,
  ...
  getSummary(pool: AccountPool): UsageSummary {
    ...
    return {
      ...
      total_estimated_cost_usd: (this.baseline.estimated_cost_usd ?? 0) + live.estimated_cost_usd,
  ...
  getHistory(
    ...
    for (let i = 1; i < filtered.length; i++) {
      ...
      deltas.push({
        ...
        estimated_cost_usd: Math.max(0, (curr.estimated_cost_usd ?? 0) - (prev.estimated_cost_usd ?? 0)),
...
function bucketize(deltas: UsageDataPoint[], bucketMs: number): UsageDataPoint[] {
  ...
  for (const d of deltas) {
    ...
    if (existing) {
      ...
      existing.estimated_cost_usd += d.estimated_cost_usd;
