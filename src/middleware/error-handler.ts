...
function formatCaughtError(err: unknown, c: Context): Response {
  const thrown = err instanceof Error ? err : new Error(String(err));
  const errRecord = thrown as unknown as Record<string, unknown>;
  const message = thrown.message || "Internal server error";
  console.error("[ErrorHandler]", thrown.stack ?? message);
  ...
  const isSyntaxError = thrown instanceof SyntaxError || thrown.name === "SyntaxError" || String(thrown).includes("SyntaxError");
...
/**
 * 同时支持 `app.onError(errorHandler)`（上游签名）和 `app.use("*", errorHandler)`
 * （本仓 e2e / 既有中间件用法）。两种调用点在 merge 后都还在。
 */
export function errorHandler(err: Error, c: Context): Response;
export function errorHandler(c: Context, next: Next): Promise<void | Response>;
export function errorHandler(
  errOrContext: unknown,
  contextOrNext: Context | Next,
): Response | Promise<void | Response> {
  if (typeof contextOrNext === "function") {
    const c = errOrContext as Context;
    const next = contextOrNext;
    return (async () => {
      try {
        await next();
      } catch (err) {
        return formatCaughtError(err, c);
      }
    })();
  }
  return formatCaughtError(errOrContext, contextOrNext);
}
