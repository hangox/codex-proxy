...
- 修复关闭 opaque compact 开关无法止血：marker 存在但 `claude_code_opaque_compact_experimental` 已关闭时，此前仍会 409（"support is disabled"），运维唯一的非回滚止血阀因此形同虚设——关掉开关只是把 409 的文案从"不可用"换成"已关闭"，带旧 marker 的会话照样报错，事故最终只能靠回滚镜像收场。现在忽略 marker、按普通请求继续（marker 原样作为文本转发上游，这是关开关的已知代价，不做恢复）。同步改写 `tests/e2e/messages.test.ts` 里把这个 bug 编码成规格的用例：原断言"关开关后仍 409 + 文案含 support is disabled"改为断言 200 + 正常转发（`src/routes/messages.ts`、`tests/e2e/messages.test.ts`）
