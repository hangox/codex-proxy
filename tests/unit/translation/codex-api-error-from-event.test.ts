...
describe("codexApiErrorFromEvent", () => {
  ...
  it("reclassifies a real invalid_function_parameters event (Artifact schema rejected) to 400", () => {
    // 真实交互式会话场景：上游走 WebSocket/SSE 事件路径报错，code 是
    // `invalid_function_parameters`（不在 statusForCode 任何一条分支里，兜底
    // 落到 502），message 里带着确定性的 schema 校验错误文案。502 落在 withRetry
    // 的可重试区间，也会被 Claude Code 客户端自己的退避重试放大。
    const err = codexApiErrorFromEvent({
  ...
  it("leaves an unrelated unknown code at the default 502 — no over-reclassification", () => {
    const err = codexApiErrorFromEvent({
