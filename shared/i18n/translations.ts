export const translations = {
  en: {
    ...
    generalSettingsCompactBridge: "Claude Code Compact Bridge",
    generalSettingsCompactBridgeHint: "Legacy path: rewrite classic Claude Code compaction as Codex compact + render, replaying the rendered history on every later turn. Superseded by Opaque Compact State — keep this off when that one is on. Applies immediately; disabled by default.",
        generalSettingsOpaqueCompact: "Experimental Opaque Compact State",
        generalSettingsOpaqueCompactHint: "Recommended path: compact once, persist the opaque state encrypted on disk, and return a short signed marker that still resolves after a process or container restart. When both switches are on this one wins and the classic bridge is only reached if a non-store error aborts a fresh compact, so enabling both is not recommended. Requires opaque_compact_state.keyring_file; disabled by default.",
  ...
  zh: {
    ...
    generalSettingsCompactBridge: "Claude Code 压缩桥接",
    generalSettingsCompactBridgeHint: "旧版路径：把经典 Claude Code 压缩请求改写成 Codex compact + render，之后每一轮都要重放渲染出来的历史。已被 Opaque 压缩状态取代，后者开启时请关掉本项。立即生效，默认关闭。",
        generalSettingsOpaqueCompact: "实验性 Opaque 压缩状态",
        generalSettingsOpaqueCompactHint: "推荐路径：只执行一次 compact，opaque 状态加密落盘，返回的短签名 marker 在进程或容器重启后仍能恢复。两者都开时本项优先，只有全新 compact 撞上非存储级错误才会退回经典桥接，因此不建议同时开启。需配置 opaque_compact_state.keyring_file，默认关闭。",
