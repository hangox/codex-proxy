...
- [ ] On a genuinely fresh deployment (new volumes, first time `claude_code_opaque_compact_experimental` is turned on): the master keyring does not bootstrap itself. `allowKeyringBootstrap` is never `true` on any production code path (`9b2763a`, intentional — auto-generating a master key is an irreversible operation and shouldn't happen without an explicit human action). Run `npm run opaque:bootstrap-keyring -- --yes` **once**, as the same user the server process runs as (`docker exec -u node <container> ...`, not bare `docker exec` — a root-created keyring file fails `assertKeyringFileSafe`'s ownership check on every later load). **`/health` will not update on its own after the script finishes** — `runtimeStore` is a module-level singleton resolved once at process startup, there is no file-watcher or polling that re-checks it. Qa confirmed by real end-to-end deployment: `/health` stayed `key_unavailable` immediately after the script succeeded, and only flipped to `ready:true` after one of the two actions below (verified directly, not inferred from reading the code):
  - **Restart the container/process** (`docker restart <container>` re-runs `index.ts`'s `startOpaqueCompactRuntime` from scratch), or
  - **Reload via Admin without restarting**: `reconfigureOpaqueCompactRuntime()` (`admin/settings.ts`'s `POST /admin/general-settings`, triggered whenever the request body includes `claude_code_opaque_compact_experimental`) calls `closeCurrentOpaqueCompactRuntime()` then `startOpaqueCompactRuntime()` again in the same process — verified directly (not by reasoning about the code) that toggling the switch off then back on in Admin picks up a keyring file that was created after the process started, with no restart needed. On production this is likely the more convenient of the two.
