...
function extractCompletedOutputImage(event: ExtractedEvent): ImageGenerationCall | null {
  ...
  if (!isRecord(data) || !isRecord(data.response) || !Array.isArray(data.response.output)) {
    return null;
  }
...
export async function collectImageGenerationResponse(options: {
  ...
  for await (const event of iterateCodexEvents(options.api, options.response)) {
    ...
    if (event.typed.type === "response.completed") {
      ...
      const data = event.raw.data;
      // Some upstream responses expose the completed output only on the earlier
      // output_item.done event. If response.completed carries an explicit output
      // array, however, it is authoritative: an empty/missing image there must
      // not let a stale intermediate result count as success.
      if (isRecord(data) && isRecord(data.response) && Array.isArray(data.response.output)) {
